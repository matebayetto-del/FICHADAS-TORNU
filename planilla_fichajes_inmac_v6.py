...
- Resalta únicamente la celda del nombre (columna E): escala roja por cantidad de faltas y amarillo por posibles ambigüedades de nombre/legajo.
"""
...
# Resaltados visuales de control.
# Solo se pinta la celda del nombre (columna detectada como APELLIDO Y NOMBRE).
UMBRAL_FALTAS_ROJO = 3

# Escala: a mayor cantidad de AUSENTE, rojo más intenso.
ESCALA_ROJO_FALTAS = [
...
# Posible error de vínculo nombre/legajo.
FILL_MATCH_AMARILLO = PatternFill("solid", fgColor="FFF2CC")
...
def _fill_rojo_por_cantidad(cantidad: int):
...
def _normalizar_legajo_control(valor):
    if valor is None:
        return ""
...
def detectar_vinculos_a_revisar(personas, cargados):
    """Devuelve {fila: [motivos]} para coincidencias potencialmente ambiguas.

    Se considera revisable cuando:
    - el método no fue legajo+nombre;
    - el puntaje fue menor a 1;
    - el legajo del fichador no coincide con el de la planilla;
    - el mismo legajo aparece en más de una persona de la planilla.

    Esto no cambia el vínculo ni las horas; solo genera una alerta visual amarilla.
    """
    cantidad_por_legajo = defaultdict(int)
    for p in personas:
        leg = _normalizar_legajo_control(p.get("legajo"))
        if leg:
            cantidad_por_legajo[leg] += 1

    revisar = defaultdict(list)
    ...
    for item in cargados:
        ...
        try:
            puntaje = float(item.get("PUNTAJE"))
        except Exception:
            puntaje = 0.0

        leg_fichador = _normalizar_legajo_control(item.get("LEG_FICHADOR"))
        leg_planilla = _normalizar_legajo_control(item.get("LEG_PLANILLA"))

        if metodo != "legajo+nombre":
            revisar[fila].append(f"Método de vínculo: {item.get('METODO')}")
        if puntaje < 0.999:
            revisar[fila].append(f"Coincidencia de nombre parcial ({puntaje:.3f})")
        ...
        if leg_planilla and cantidad_por_legajo.get(leg_planilla, 0) > 1:
            revisar[fila].append(
                f"Legajo {leg_planilla} repetido en la planilla; se diferenció por nombre"
            )

    return revisar


def aplicar_resaltados_alertas(
    ws,
    personas,
    bloques,
    anio,
    ...
    """Aplica resaltados SOLO en la celda del nombre.

    Rojo con intensidad variable:
        Según cantidad de días cuyo estado diario sea exactamente AUSENTE.

    Amarillo:
        Cuando el emparejamiento entre fichador y planilla puede requerir revisión
        por nombre parcial, método sin legajo+nombre, legajo distinto o legajo
        repetido en la planilla.

    Prioridad visual:
        El amarillo tiene prioridad sobre el rojo porque primero debe verificarse
        que las horas pertenezcan a la persona correcta. La cantidad de faltas
        continúa informada en la hoja ALERTAS.
    """
    alertas = []
    dias_mes = calendar.monthrange(anio, mes)[1]
    revisar_por_fila = detectar_vinculos_a_revisar(personas, cargados)
    ...
    for p in personas:
        ...
        for dia in range(1, min(dias_mes, len(bloques)) + 1):
            ...
            if estado == "AUSENTE":
                faltas_dias.append(fecha)

        fill_rojo, nivel_rojo = _fill_rojo_por_cantidad(len(faltas_dias))
        razones_revision = revisar_por_fila.get(fila, [])

        # Primero rojo según faltas.
        if fill_rojo is not None:
            ...
            alertas.append({
                ...
                "CANTIDAD": len(faltas_dias),
                "DETALLE": ", ".join(f.strftime("%d/%m/%Y") for f in faltas_dias),
                "CRITERIO": f"{len(faltas_dias)} días con AUSENTE",
            })

        # Amarillo tiene prioridad cuando hay posible error de identificación.
        if razones_revision:
            ws.cell(fila, cols["col_nombre"]).fill = copy.copy(FILL_MATCH_AMARILLO)
            alertas.append({
                "TIPO": "REVISAR_NOMBRE_LEGAJO",
                "COLOR": "AMARILLO",
                "FILA": fila,
                "LEG": p.get("legajo"),
                "PERSONA": p.get("nombre"),
                "CANTIDAD": len(razones_revision),
                "DETALLE": " | ".join(razones_revision),
                "CRITERIO": "Posible ambigüedad durante el vínculo nombre/legajo",
            })
...
def procesar_archivos(archivo_planilla, archivo_fichajes, descargar_en_colab=False):
    ...
    # Resaltados visuales de control: rojo suave por muchas faltas y verde suave por fichadas dudosas.
    alertas = aplicar_resaltados_alertas(ws, personas, bloques, anio, mes, feriados, max_fecha, marcas, cols, cargados)

    agregar_hoja_control(wb, "VINCULOS_CARGADOS", cargados)
    agregar_hoja_control(wb, "IGNORADOS_FICHADOR", ignorados)
    agregar_hoja_control(wb, "ALERTAS", alertas)
    agregar_hoja_control(wb, "DIAGNOSTICO", [{
        ...
        "ALERTAS_MUCHAS_FALTAS": sum(1 for a in alertas if a.get("TIPO") == "MUCHAS_FALTAS"),
        "ALERTAS_REVISAR_NOMBRE_LEGAJO": sum(1 for a in alertas if a.get("TIPO") == "REVISAR_NOMBRE_LEGAJO"),
        "UMBRAL_FALTAS_ROJO": UMBRAL_FALTAS_ROJO,
